...
public class DrawRewardInfoService {
  ...
  public DrawRewardInfoListResponse getAllDrawRewardInfo() {
    ...
    List<DrawRewardInfoListResponse.DrawRewardInfoResponse> rewardInfoResponses = rewardInfos.stream()
                                                                                             .map(
                                                                                                 rewardInfo -> new DrawRewardInfoListResponse.DrawRewardInfoResponse(
                                                                                                     rewardInfo.getRanking(),
                                                                                                     rewardInfo.getName(),
                                                                                                     rewardInfo.getStock(),
                                                                                                     rewardInfo.getImage()))
                                                                                             .collect(
  ...
  public void setDrawRewardInfoList(DrawRewardInfoListRequest request) {
    List<DrawRewardInfo> rewardInfoList = request.getRewards().stream().map(rewardInfoRequest -> {
      if (rewardInfoRequest.getImage() == null) {
        DrawRewardInfo drawRewardInfo = drawRewardInfoRepository.findById(rewardInfoRequest.getRanking())
                                                                .orElseThrow(
        ...
        return new DrawRewardInfo(rewardInfoRequest.getRanking(), rewardInfoRequest.getName(),
            rewardInfoRequest.getStock(), drawRewardInfo.getImage());
      }
